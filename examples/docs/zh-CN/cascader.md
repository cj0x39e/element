...
```html
<el-cascader v-model="value" :props="props" collapse-tags placeholder="请选择好啊阿萨德发送到"></el-cascader>
      ...
      return {
        ...
        props: {
          ...
          lazyLoad (node, resolve) {
            ...
            const getValue = (level, index) => {
              if (level <= 1) {
                return [[1], [2, 3]][level][index]
              } else {
                return [[4, 5, 6], [7, 8, 9]][value - 2][index]
              }
            }
            setTimeout(() => {
              const nodes = Array.from({ length: level + 1 })
                .map((item, index) => ({
                  value: getValue(level, index),
                  label: `选项${getValue(level, index)}`,
                  leaf: level >= 2
                }));
                console.log(nodes)
